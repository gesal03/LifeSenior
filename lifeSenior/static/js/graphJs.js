...
if (allScoreGraphViews.length > 0) {
  ...
  allScoreGraphViews.forEach(function(allScoreGraphView, index) {
    ...
    allScoreGraphView.style.height = `${(500 / 6) * allScoreDataValue}px`; // 데이터 값을 픽셀 단위로 높이로 설정
  });
  //마지막 그래프 색깔 변경
...
if (notAllScoreDataValues.length > 0) {
  ...
  notAllScoreGraphViews.forEach(function(notAllScoreGraphView, index) {
    ...
    notAllScoreGraphView.style.height = `${(500 / 5) * notAllScoreDataValue}px`; // 데이터 값을 픽셀 단위로 높이로 설정
  });
  //마지막 그래프 색깔 변경
